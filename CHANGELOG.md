...
- Matomo Tracking API type-checks leftover JSON `_cvar` / `cvar` /
  `uadata`, `recMode` `1`/`2` (bot-only requires `ua`), `bots=1`
  (forbidden with `recMode`), and integer `http_status` / `bw_bytes`.
...
- TikTok Events API 1.0 type-checks `properties.contents[]` item `content_id`,
  `content_type` (`product` or `product_group`), `quantity`, unit `price`,
  `content_name`, `content_category`, and `status` from the official SDK.
  `content_type` is not contracted on `properties`. `status` has no enumerated
  values.
